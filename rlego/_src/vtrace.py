...
def vtrace(v_tm1: Tensor, v_t: Tensor, r_t: Tensor, discount_t: Tensor, rho_tm1: Tensor, lambda_: float = 1.,
    ...
    errors = importance_corrected_td_errors(r_t, discount_t, c_tm1, torch.cat([v_tm1, v_t[-1:]], dim=0),
                                            lambda_=lambda_)
    return errors + v_tm1
...
def vtrace_td_error_and_advantage(v_tm1: Tensor, v_t: Tensor, r_t: Tensor, discount_t: Tensor, rho_tm1: Tensor,
    ...
    with torch.no_grad():
        target_tm1 = vtrace(v_tm1, v_t, r_t, discount_t, rho_tm1, clip_rho_threshold=clip_rho_threshold,
                            lambda_=lambda_)
    ...
    return VTraceOutput(adv, target_tm1 - v_tm1, q_estimate)
