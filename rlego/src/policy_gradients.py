...
def policy_gradient(pi_t: torch_dist.Distribution, a_t: T, q_t: T) -> T:
    action_log_prob = pi_t.log_prob(a_t.detach()).sum(dim=-1)
...
def mdpo(pi_tm1: torch_dist.Distribution, pi_t: torch_dist.Distribution, a_t: T, adv: T) -> T:
    rho_t = (pi_tm1.log_prob(a_t) - pi_t.log_prob(a_t)).sum(dim=-1).exp()
    kl = torch.distributions.kl_divergence(pi_tm1, pi_t).sum(dim=-1)
    return (rho_t * adv), kl
